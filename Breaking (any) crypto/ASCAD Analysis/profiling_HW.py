...
for i, hw in enumerate(profiling_labels):
    hw_val = HW_table[hw]
    hw_groups[hw_val].append(i)

for hw_val in range(HW_CLASSES):
    indices = hw_groups[hw_val]
    if len(indices) == 0:
        # If no traces exist for this HW, use defaults.
        mean_vectors[hw_val] = np.zeros(profiling_traces.shape[1], dtype=np.float32)
        covariance_matrices[hw_val] = np.eye(profiling_traces.shape[1], dtype=np.float32)
    else:
        traces_group = profiling_traces[indices, :]
        mean_vectors[hw_val] = np.mean(traces_group, axis=0)
        covariance_matrices[hw_val] = np.cov(traces_group, rowvar=False) + lambda_reg * np.eye(profiling_traces.shape[1])

print("Hamming Weight templates built.")
