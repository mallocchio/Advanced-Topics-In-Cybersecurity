...
def bit_flip(binary_str, bit_index):
...
def encryption(user_input, fault=False, position=0):
    ...
    for round_num in range(16): 
        ...
        if fault and round_num == 15:
            rpt = bit_flip(rpt, position)
...
# Draft a script based on the available DES functions to simulate fault injection and recover `K16`

def reverse_cipher(c):
    # print("reverse_cipher", c)
    c_bin = ip_on_binary_rep(str_to_bin(c))
    l16 = c_bin[32:]
    r16 = c_bin[:32]
    r15 = l16

    inverse_p_box_table = [0] * len(p_box_table)
    for i, p in enumerate(p_box_table):
        inverse_p_box_table[p - 1] = i + 1

    unpermuted = ''.join([r16[i-1] for i in inverse_p_box_table])
    output = unpermuted[:4]

    expanded_result = [r15[i - 1] for i in e_box_table]
    expanded_result_str = ''.join(expanded_result)

    input = expanded_result_str[:6]

    return input, output

# Override of the function to recover the last round  key
def LRK_reverse_cipher(c, ind):
    # print("reverse_cipher", c)
    c_bin = ip_on_binary_rep(str_to_bin(c))
    l16 = c_bin[32:]
    r16 = c_bin[:32]
    ...
    unpermuted = ''.join([r16[i-1] for i in inverse_p_box_table])
    output = unpermuted[(ind-1)*4:ind*4]

    expanded_result = [r15[i - 1] for i in e_box_table]
    expanded_result_str = ''.join(expanded_result)

    input = expanded_result_str[(ind-1)*6:ind*6]

    return input, output


def apply_s_box(c_in):
    s_box_substituted = ''

    row_bits = int(c_in[0] + c_in[-1], 2)
    col_bits = int(c_in[1:-1], 2)

    s_box_value = s_boxes[0][row_bits][col_bits]

    s_box_substituted += format(s_box_value, '04b')

    return s_box_substituted

# Override of the function for recovering the last round key
def LRK_apply_s_box(c_in, ind):
    s_box_substituted = ''

    row_bits = int(c_in[0] + c_in[-1], 2)
    col_bits = int(c_in[1:-1], 2)

    s_box_value = s_boxes[ind-1][row_bits][col_bits]
...
# Decomment and indent for the last round key recovery 
if __name__ == "__main__":
    ...
    #ind = 1
    #iteration = 0
    #positions = [31, 0, 1, 2, 3, 4, 3, 4, 5, 6, 7, 8, 7, 8, 9, 10, 11, 12, 11, 12, 13, 14, 15, 16, 15, 16, 17, 18, 19, 20, 19, 20, 21, 22, 23, 24, 23, 24, 25, 26, 27, 28, 27, 28, 29, 30, 31, 0]

    #for position in positions:
    c = encryption(m)
    c_in, c_out = reverse_cipher(c)
    #c_in, c_out = LRK_reverse_cipher(c, ind)
    c_fault = encryption(m, True)
    #c_fault = encryption(m, True, position)
    c_fault_in, c_fault_out = reverse_cipher(c_fault)
    #c_fault_in, c_fault_out = LRK_reverse_cipher(c_fault, ind)

    for k16 in range(2 ** 6):
        round_key_str = format(k16, '06b')

        c_xor_result_str = ''
        c_fault_xor_result_str = ''
        for i in range(6):
            c_xor_result_str += str(int(c_in[i]) ^ int(round_key_str[i]))
            c_fault_xor_result_str += str(int(c_fault_in[i]) ^ int(round_key_str[i]))

        c_s_box = apply_s_box(c_xor_result_str)
        #c_s_box = LRK_apply_s_box(c_xor_result_str, ind)
        c_fault_s_box = apply_s_box(c_fault_xor_result_str)
        #c_fault_s_box = LRK_apply_s_box(c_fault_xor_result_str, ind)

        right_equation = ''.join([str(int(c_s_box[i]) ^ int(c_fault_s_box[i])) for i in range(4)])

        left_equation = ''.join([str(int(c_out[i]) ^ int(c_fault_out[i])) for i in range(4)])

        if left_equation == right_equation:
            possible_keys.append(round_key_str)

        #if iteration == (6 * ind) - 1:
        #    c = Counter(possible_keys) 
        #    recovered_key.append(c.most_common()[0])
        #    possible_keys = []
        #    ind += 1
